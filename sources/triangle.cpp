...
bool Triangle::contains_vertex(unsigned int num) const
{
  for (int i = 0; i < n_vertices; ++i)
    if (_vertices[i] == num)
      return true;
...
void Triangle::ghost_cells(const std::vector<unsigned int> &g_cells)
{
  _ghost_cells.clear(); // clear old list, if any
  _ghost_cells = g_cells;
}
